...
function commandCategory(command) {
  ...
  const groups = [
    ...
    ['🎮 JUEGOS', /^(dado|moneda|ppt|numero|quiz|reto|verdad|pregunta|juego|juegos|eleccion|decision|random|suerte|robar|boss|aventura)/],
    ...
    ['⭐ NIVEL & XP', /^(xp|nivel|level|levelup|rank|ranking|rango|topxp|racha|logro|logros|progreso|estadisticas|stats|top)/],
    ['🛠️ UTILIDADES', /^(ping|pong|hora|fecha|fecha2|fechaactual|calc|calculadora|say|qr|clima|temperatura|codigo|texto|emoji|randomemoji|archivo|array|async|await|funcion|variable|if|loop|math|objeto|json|javascript|js|html|css|node|nodejs|api|test|check|debug|error|fix|numero2)/],
...
function commandMenu() {
  ...
  const sections = order.filter(c => grouped[c]?.length).map(category => {
    ...
    for (let i = 0; i < list.length; i += 2) {
      const left = `✦ ${PREFIX}${list[i]}`;
      const right = list[i + 1] ? ` ✦ ${PREFIX}${list[i + 1]}` : '';
