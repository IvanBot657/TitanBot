...
const PAIRING_METHOD = String(process.env.PAIRING_METHOD || "both").toLowerCase();
...
function askPairingNumber() {
  return new Promise(resolve => {
    ...
    rl.question("📱 Escribe tu número de WhatsApp (con código de país, solo números): ", answer => {
      rl.close();
...
async function startBot() {
  try {
    ...
    sock.ev.on("connection.update", async update => {
      try {
        ...
        if (qr && !state.creds.registered) {
          if (PAIRING_METHOD === "qr" || PAIRING_METHOD === "both") {
            console.log("================================");
            console.log("📷 QR DE VINCULACIÓN DISPONIBLE");
            console.log("📱 WhatsApp Business → Dispositivos vinculados → Vincular dispositivo");
            console.log("👆 Escanea el QR mostrado arriba en los Logs.");
            console.log("================================");
          }

          if ((PAIRING_METHOD === "number" || PAIRING_METHOD === "both") && !pairingCodeRequested) {
            pairingCodeRequested = true;

            let number = PAIRING_NUMBER.replace(/\D/g, "");

            // Si no se configuró PAIRING_NUMBER en Render/entorno,
            // pedir el número directamente por la terminal.
            if (!number) {
              number = await askPairingNumber();
            }

            if (!number) {
              pairingCodeRequested = false;
              console.error("❌ No se recibió un número válido.");
            } else {
              try {
                ...
                console.log("Dispositivos vinculados → Vincular dispositivo → Vincular con número de teléfono");
                console.log("================================");
              } catch (error) {
                pairingCodeRequested = false;
    ...
    if (!state.creds.registered) {
      ...
      if (PAIRING_METHOD === "number" || PAIRING_METHOD === "both") {
        console.log("🔢 Se pedirá el número en la terminal si no existe PAIRING_NUMBER.");
      }
      if (PAIRING_METHOD === "qr" || PAIRING_METHOD === "both") {
        console.log("📷 También se mostrará el QR en los Logs/terminal.");
      }
    }
  } catch (error) {
...
console.log(`🤖 ${BOT_NAME}`);
console.log(`⚡ ${FRASE}`);
console.log(`📱 Vinculación: ${PAIRING_METHOD.toUpperCase()}`);
console.log("🟢 QR: ACTIVADO");
console.log("🔢 CÓDIGO POR NÚMERO: ACTIVADO SI HAY PAIRING_NUMBER");
console.log("================================");
