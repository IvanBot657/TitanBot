...
async function startBot() {
  try {
    ...
    if (!state.creds.registered) {
      ...
      console.log("📷 El QR se mostrará en los Logs/terminal.");
    }
  } catch (error) {
...
http.createServer((req, res) => {
  ...
  if (url.pathname === "/api/qr") {
    ...
    res.end(JSON.stringify({ qr: currentQR, status: botConnection }));
    return;
  }
...
<style>
*{box-sizing:border-box}body{margin:0;min-height:100vh;background:#0b0f12;color:#fff;font-family:Arial,sans-serif;display:flex;justify-content:center;align-items:center;padding:20px}.card{width:min(92vw,430px);background:#151a1f;border:1px solid #293139;border-radius:22px;padding:26px;text-align:center;box-shadow:0 15px 45px rgba(0,0,0,.4)}h1{margin:0 0 8px;font-size:28px}p{color:#aeb7bf;line-height:1.45}.qrbox{width:300px;height:300px;max-width:100%;margin:20px auto;background:#fff;border-radius:14px;display:flex;align-items:center;justify-content:center;overflow:hidden}.qrbox canvas{max-width:100%;height:auto}.status{font-weight:700;margin:12px 0}.steps{text-align:left;background:#0f1317;border-radius:14px;padding:14px 18px;color:#d9e0e5}.steps li{margin:9px 0}.hidden{display:none}.ok{font-size:48px}</style>
</head>
<body>
<div class="card">
<h1>🤖 TITANBOT</h1>
...
<div id="qrbox" class="qrbox"><span id="message">⏳ Generando QR...</span></div>
<ol class="steps">
...
async function updateQR(){
  try{
    const r=await fetch('/api/qr?t='+Date.now(),{cache:'no-store'});
    ...
    if(data.qr && data.qr!==lastQR){
      ...
      await QRCode.toCanvas(canvas,data.qr,{width:280,margin:2});
      statusEl.textContent='📷 Escanea el QR con WhatsApp';
    }else if(!data.qr){
      statusEl.textContent=data.status==='connecting'?'🔄 Conectando...':'⏳ Esperando un nuevo QR...';
      if(!lastQR) message.textContent='⏳ Esperando QR...';
    }
  }catch(e){
    statusEl.textContent='⚠️ No se pudo consultar el QR';
  }
}
updateQR();
