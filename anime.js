...
async function buscarImagenPersonaje(nombre, intentos = 3) {
  const nombreCodificado = encodeURIComponent(nombre);

  for (let intento = 1; intento <= intentos; intento++) {
    try {
      console.log(`🔎 Jikan: buscando ${nombre} (intento ${intento}/${intentos})...`);

      const respuesta = await fetch(
        `${JIKAN_API}/characters?q=${nombreCodificado}&limit=10`
      );

      if (!respuesta.ok) {
        if ([408, 429, 500, 502, 503, 504].includes(respuesta.status) && intento < intentos) {
          const espera = intento * 2000;
          console.log(`⏳ Jikan respondió ${respuesta.status}. Reintentando en ${espera / 1000}s...`);
          await new Promise(resolve => setTimeout(resolve, espera));
          continue;
        }
        throw new Error(`Jikan respondió HTTP ${respuesta.status}`);
      }

      const datos = await respuesta.json();

      if (!datos?.data || datos.data.length === 0) return null;

      const nombreNormalizado = nombre.toLowerCase().trim().replace(/\s+/g, " ");

      const encontrado =
        datos.data.find(personaje =>
          String(personaje?.name || "")
            .toLowerCase()
            .trim()
            .replace(/\s+/g, " ") === nombreNormalizado
        ) || datos.data[0];

      const personajeId = encontrado?.mal_id;
      if (!personajeId) return null;

      const respuestaCompleta = await fetch(
        `${JIKAN_API}/characters/${personajeId}/full`
      );

      if (!respuestaCompleta.ok) {
        if ([408, 429, 500, 502, 503, 504].includes(respuestaCompleta.status) && intento < intentos) {
          const espera = intento * 2000;
          console.log(`⏳ Jikan falló (${respuestaCompleta.status}). Reintentando en ${espera / 1000}s...`);
          await new Promise(resolve => setTimeout(resolve, espera));
          continue;
        }
        throw new Error(`Jikan respondió HTTP ${respuestaCompleta.status}`);
      }

      const completo = await respuestaCompleta.json();

      const imagen =
        completo?.data?.images?.jpg?.image_url ||
        completo?.data?.images?.jpg?.small_image_url ||
        completo?.data?.images?.webp?.image_url ||
        completo?.data?.images?.webp?.small_image_url ||
        encontrado?.images?.jpg?.image_url ||
        encontrado?.images?.webp?.image_url ||
        null;

      if (!imagen) return null;

      const imagenValida = await descargarImagen(imagen);

      if (!imagenValida) {
        if (intento < intentos) {
          const espera = intento * 2000;
          console.log(`⏳ Reintentando descarga en ${espera / 1000}s...`);
          await new Promise(resolve => setTimeout(resolve, espera));
          continue;
        }
        return null;
      ...
      console.log(`✅ Imagen encontrada para ${nombre}.`);
      return imagen;
    } catch (error) {
      console.error(`Error buscando imagen de ${nombre} (intento ${intento}/${intentos}):`, error);

      if (intento < intentos) {
        const espera = intento * 2000;
        console.log(`🔄 Reintentando Jikan en ${espera / 1000}s...`);
        await new Promise(resolve => setTimeout(resolve, espera));
      }
    }
  }

  console.log(`❌ Jikan no respondió después de ${intentos} intentos para ${nombre}.`);
  return null;
}
